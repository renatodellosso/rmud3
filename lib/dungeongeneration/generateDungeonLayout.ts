...
export default function generateDungeonLayout(): Dungeon {
  ...
  for (let depth = 0; depth <= maxDepth; depth++) {
    ...
    throwIfRoomsMissing(
      `${generateDungeonLayout.name} falsely claimed to have generated rooms at depth ${depth}.`,
      dungeon,
      depth,
      startingPoints
    );
...
function selectExits(floor: FloorInstance, rooms: DungeonLocation[]): Point[] {
  const exitRange = floor.definition.layoutGenerationOptions.exitCount;
...
function generateFloorDimensionsAndOffsetStartingPoints(
  ...
  const width = Math.max(
    randInRangeInt(
      definition.layoutGenerationOptions.width[0],
      definition.layoutGenerationOptions.width[1]
    ),
    maxWidthBetweenStartingPoints
  );
  const length = Math.max(
    randInRangeInt(
      definition.layoutGenerationOptions.length[0],
      definition.layoutGenerationOptions.length[1]
    ),
...
function generateFloorLayout(
  ...
  const options = floorInstance.definition.layoutGenerationOptions;
